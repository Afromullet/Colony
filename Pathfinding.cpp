...
std::map<GridLocation,GridLocation> aStarSearch(Map &map, GridLocation start, GridLocation end)
{
 ...
 //std::cout << "\nCost so far size " << costSoFar.size();
    while(!frontier.empty())
    {
        ...
        neighbors = GetPNeighbors(sf::Vector2i(current.x,current.y),map,false);
...
std::vector<sf::Vector2i> GetPNeighbors(sf::Vector2i startPoint, Map &map,bool getNonPassable)
{
