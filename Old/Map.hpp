...
//When on a particular tile, we must determine its position
//Think of some way to determine which tile efficiently (searching for quads sounds inefficeint.
...
private:
    ...
    //This vector is basically a bag of cells we can use to create the map
...
/*
 bool loadCreature(const std::string& tileset, sf::Vector2u _tileSize, sf::Vector2u creaturePosition);

 std::vector<Tile> tiles;
 Old. here for historical purposes
 void BasicRandomMap(sf::Vector2u _tileSize,unsigned int _width, unsigned int _height);
 void RandomCaveMap(sf::Vector2u _tileSize,unsigned int _width, unsigned int _height,int aliveTileID, int deadTileId, float chanceToStartAlive, int NumberOfSteps,int birthLimit, int deathLimit);
 
 void SimulationStep(int aliveTileID, int deadTileID,int birthLimit, int deathLimit);
 int GetLivingNeighbors(int aliveTileID, int deadTileID, int x, int y);
 
 void TestRuleset();
 void TestRuleset2();
 bool GenerateTileQuads (sf::Vector2u _tileSize,unsigned int _width, unsigned int _height);
 void GroupTiles();
 
 void Random2DCaveMap(sf::Vector2u _tileSize,unsigned int _width, unsigned int _height,int aliveTileID, int deadTileId, float chanceToStartAlive, int NumberOfSteps,int birthLimit, int deathLimit);
 
 void GenerateRandomTiles();
 void Copy2DMapToVector();
 
   sf::Texture m_tileset;
 virtual void drawCreature(sf::RenderTarget& target, sf::RenderStates states) const;
