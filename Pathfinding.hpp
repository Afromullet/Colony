...
std::vector<sf::Vector2i> GetPNeighbors(sf::Vector2i startPoint, Map &map,bool getNonPassable = true); //Returns the indices of the neighboring cells, with cells that are not in bounds removed. Optional argument is to determine whether to select tiles creature can pass
